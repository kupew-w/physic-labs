#include "labs/lab2(1)/Lab2.h"
#include "Calc.h"
#include "labs/lab2(1)/Lab2Calc.h"
#include "labs/lab2(1)/Lab2Const.h"
#include "labs/lab2(1)/Lab2Error.h"
#include "labs/lab2(1)/Lab2Exp.h"
#include "tokens/lab/ExperimentToken.h"
#include "tokens/lab/LabToken.h"
#include <cmath>
#include <cstdlib>
#include <memory>

Lab2::Lab2(std::shared_ptr<pl::LabToken> token)
{
    for(std::shared_ptr<pl::ExperimentToken> expToken : *token)
    {
        raw.push_back(std::make_shared<Lab2Exp>(expToken));
        calc.push_back(std::make_shared<Lab2Calc>());
        errors.push_back(std::make_shared<Lab2Error>());
    }

    constants = std::make_shared<Lab2Const>();
}

void Lab2::calculateLab()
{
    std::vector<double> I;
    std::vector<double> U;
    for(std::shared_ptr<ExperimentStruct> exp : raw)
    {
        std::shared_ptr<Lab2Exp> expLab = std::static_pointer_cast<Lab2Exp>(exp);
        I.push_back(expLab->I);
        U.push_back(expLab->U);
    }

    std::shared_ptr<Lab2Exp> expLab = std::static_pointer_cast<Lab2Exp>(raw[0]);
    std::shared_ptr<Lab2Calc> calcLab = std::static_pointer_cast<Lab2Calc>(calc[0]);
    std::shared_ptr<Lab2Const> constLab = std::static_pointer_cast<Lab2Const>(constants);

    calcLab->Ia = Calc::average(I);
    calcLab->Ua = Calc::average(U);
    constLab->Ri = std::abs(Calc::coeffA(I, U));
    //calcLab->Re = expLab->U0 - calcLab->Ia * constLab->Ri;
    calcLab->ShCr = expLab->U0/constLab->Ri;
    calcLab->P0 = expLab->U0*calcLab->Ia;
    calcLab->Pi = calcLab->Ia*calcLab->Ia*constLab->Ri;
    calcLab->Eff = (calcLab->P0-calcLab->Pi)/calcLab->P0;

    std::shared_ptr<Lab2Error> errorLab = std::static_pointer_cast<Lab2Error>(errors[0]);

    errorLab->U = Calc::randomMiss(U);
    errorLab->I = Calc::randomMiss(I);
    errorLab->Ri = std::abs(Calc::deltaCoeffA(I, Calc::dy(calcLab->Ua, expLab->U0-calcLab->Ia*constLab->Ri)));
    //errorLab->Re = sqrt(pow(constLab->Ri*errorLab->I, 2)+
    //                    pow(calcLab->Ia*errorLab->Ri, 2));
    errorLab->ShCr = std::abs(expLab->U0/pow(constLab->Ri, 2)*errorLab->Ri);
    errorLab->P0 = std::abs(expLab->U0*errorLab->I);
    errorLab->Pi = sqrt(pow(calcLab->Ia*calcLab->Ia*errorLab->Ri, 2)+
                        pow(2*pow(calcLab->Ia, 3)*constLab->Ri*errorLab->I, 2));
    errorLab->Eff = sqrt(pow(errorLab->Pi/calcLab->P0, 2)+
                         pow(calcLab->Pi*calcLab->P0*calcLab->P0*errorLab->P0, 2));
}

